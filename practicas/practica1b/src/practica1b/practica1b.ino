...
#define MIN_NUM_LOAD  200
#define MAX_NUM_LOAD  280 // con un threadPeriod_ms de 200
...
static int getRandomNumber(int min = MIN_NUM_LOAD , int max = MAX_NUM_LOAD);
static int getRandomNumber(int min, int max){
  return rand()%(max-min + 1) + min;
}
volatile int currentLoad1 = getRandomNumber();
volatile int currentLoad2 = getRandomNumber();
volatile int currentLoad3 = getRandomNumber();

static int getCurrentPeriod( int currentLoad ){
  return (currentLoad * 200) / 280;
}

volatile uint32_t threadPeriod_ms[NUM_THREADS] = { CYCLE_MS, 
                                                  getCurrentPeriod(currentLoad1), 
                                                  getCurrentPeriod(currentLoad2), 
                                                  getCurrentPeriod(currentLoad3), 0 };

volatile int threadLoad[NUM_THREADS] = { 0, currentLoad1, currentLoad2, currentLoad3, 0};
